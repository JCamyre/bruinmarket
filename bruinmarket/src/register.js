...
const register = async (
  username,
  email,
  password //returns false if registration is in valid (e.g. if account already exists)
) => {
  if (!(email.endsWith("@g.ucla.edu") /*|| email.endsWith("@ucla.edu")*/)) {
    return -1;
  }
  try {
    const result = await authentication.createUserWithEmailAndPassword(
      auth,
      email,
      password
    );
    ...
    await firestore
      .addDoc(firestore.collection(database, "users"), {
        uid: newUser.uid,
        username,
        authProvider: "local",
        email,
